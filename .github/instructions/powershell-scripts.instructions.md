...
1. **The numbered workstation family** — `Environment\ENVIRONMENT-MONEY-INSTALL\00`–`05` — uses the modern `Show-*` + emoji style below. New scripts in this family must follow it.
...
1. **Re-declare the `Show-*` helpers verbatim** — `Show-Section`, `Show-Info`, `Show-Warning`, `Show-Error`, `Show-Success`. They are deliberately duplicated per file because each script ships standalone via `iex`; **do not** refactor them into a shared module. (`00/02/03/04` use the multi-line form, `01/05` use one-line bodies — both are fine.)
2. **Gates, in this order:**
   ```powershell
   Set-ExecutionPolicy RemoteSigned -Force
   # WindowsPrincipal IsInRole "Administrator"  -> Show-Error + exit if false
   if ($PSversionTable.PsVersion.Major -lt 7) { Show-Error ...; exit }   # 01 and 03–05 only (see exceptions below)
   ```

   The PS7 gate has two deliberate exceptions in the 00–05 family: `00.PreConfig.ps1` omits it because it bootstraps PowerShell 7 itself, and `02.Driver.ps1` omits it on purpose — it sets `Tls12` for older sessions and is written to tolerate Windows PowerShell 5.1. Don't add a PS7 gate to either. (The `Show-*` helpers and the admin check are still present in all of 00–05.)
...
4. **Bare `Show-Section` blocks with no `Show-Success`/`Show-Error`** are intentional where a section is just a run of independent registry tweaks (e.g. `03.Setup01.ps1` ~lines 75–135). Don't add logging noise there — match what's present.

## `iex` / `$PSScriptRoot` — sibling-file fallback

Under `iex`, `$PSScriptRoot` is empty, so a script that needs a **sibling script or text file** must fall back to the `master` raw URL. The established pattern (from `04.Setup02.ps1`):
