...
### Added
- 01.WinUpdate.ps1 — after the Windows Update pass, best-effort **request Microsoft Store app updates** using the official **Microsoft Store CLI** (`store updates --apply`; introduced 2026-02-11, ships inside the `Microsoft.WindowsStore` package rather than as a separate install — it is the command-line counterpart of the Store's "Get updates" button, though it does not independently confirm per-app completion). Because an older Store has no CLI, a pre-step first best-effort-updates the Store app itself via winget (`--id 9WZDNCRFJBMP --source msstore`), then the CLI is **re-resolved** (with a short bounded retry for alias registration) and run. Both executables are resolved defensively for this elevated step: winget **only** from its owning `Microsoft.DesktopAppInstaller` package under the protected `C:\Program Files\WindowsApps` (never a bare PATH lookup), and `store.exe` via its App Execution Alias — required because running the physical package binary loses package identity — after confirming the `Microsoft.WindowsStore` package is installed and the alias is still a genuine 0-byte reparse-point stub (blunting a naive executable hijack in the user-writable alias directory). Everything is **non-fatal** (the whole section is wrapped so it can never block Windows Update or the reboot) and runs each child via `Start-Process` with a wall-clock **timeout** + best-effort tree-kill and stdin redirected from an empty temp file (a *bounded* unattended attempt — the Preview CLI exposes no `--silent`/`--yes`); if the Store-app pre-step times out mid-swap the CLI step is **deferred** to a later pass. Messaging is deliberately honest: the Preview CLI has no published exit-code contract, so exit 0 is reported as "completion (may include nothing available); per-app completion not independently verified" and non-zero as an unclassified/possibly-partial outcome — it never claims "all Store apps updated". Note: `store updates --apply` can update Windows Terminal itself; MSIX often defers updates for an in-use package (behavior depends on the package manifest and Windows build) and under the orchestrator 01 runs in a conhost, but a standalone Windows-Terminal-hosted run is warned it could be disrupted. If `store.exe` is unavailable (older build / Store disabled) the step warns and skips.
- 04.Setup02.ps1 — new final step `[9/9]` that disables unnecessary **startup apps** at the end of setup, tiered by host, using only reversible mechanisms. Startup entries are disabled the same way Task Manager's Startup tab does — writing the `StartupApproved` `0x03` "disabled" flag (with a FILETIME payload) under HKCU, leaving the original `Run` value / Startup-folder shortcut intact so any item can be re-enabled from **Task Manager > Startup apps**. Where an app's autostart is a **service** it is set to **Manual** (reversible to Automatic) instead. It never disables raw scheduled tasks, never writes HKLM, and never deletes anything. Each rule positively identifies the source before acting — the entry must exist, its **target executable must exist on disk and match the expected basename** (Edge's dynamic `MicrosoftEdgeAutoLaunch_*` value is matched by anchored regex + the `msedge.exe` path; ShareX's shortcut target is resolved and checked), only `absent`/well-formed-`enabled` records are transitioned (locked `0x08`/`0x09`, malformed, or unreadable records are skipped), and the written 12-byte record is re-read and verified byte-for-byte; missing entries are reported as *deferred*, not failures, and any real failure is surfaced through the step's verified-warning summary. **Policy:** all hosts disable Claude, Edge pre-launch and LINE; **powerful workstations** also disable Docker Desktop; **thin-and-light laptops** also disable ShareX, Microsoft Teams (best-effort — a new MSIX Teams `startupTask` is left for manual/Task-Manager control), OneDrive (its **Run entry only** — the separate `OneDrive Startup Task` scheduled task is left untouched, so OneDrive can still start via that task) and Microsoft **Lists** background sync (`OneDrive.Sync.Service.exe`); the **Wacom** service (`WTabletServicePro`) is set to Manual on every host except `MONEY-PC`. A report-only summary then lists the other classic startup entries not marked disabled, by name (never their command lines; excludes MSIX and scheduled tasks), for later review. Idempotent and reversible; a **point-in-time cleanup** (not persistent enforcement) — entries recreated by later app updates need a re-run.
